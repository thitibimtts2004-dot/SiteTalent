...
## T-017: Full-width layout + left sidebar navigation + system logo
- [ ] T-017 · P1 · depends_on: — · Independent-of: T-008..T-012 (layout shell only; no data/logic change)
  Title: Make every page use the full window width, move the top nav into a collapsible left sidebar, and add the system logo
  ContextTask: User request 2026-09-23. All 5 pages cap content at max-w-6xl / max-w-7xl (app/page.tsx, app/sites, app/workers, app/criticality, app/trends), leaving empty side margins on wide screens. Navigation is a horizontal top bar (components/Nav.tsx, client component using usePathname) with the DataStatusBar strip under it (app/layout.tsx). No public/ folder exists yet, so there is no logo/favicon. Plan (S1 sidebar · S2 full width · S3 logo): S1 = left sidebar with logo + "SiteTalent" + 5 nav items with icons + active-state marker, DataStatusBar moved to the sidebar footer, collapse-to-icons toggle (remembered per browser), and a ☰ drawer on small/mobile screens. S2 = drop the max-w caps so content fills the space beside the sidebar; check every chart resizes and tables stay readable. S3 = create public/, place the logo file (user to supply — SVG or transparent PNG ≥256px; ideally full logo + small icon for the collapsed state), use it in the sidebar and as the favicon; ship a placeholder until the real file arrives.
  Goal: on a 1920px screen the dashboard fills the window beside a left sidebar that shows the system logo; the sidebar collapses to icons and becomes a ☰ drawer on mobile; all 5 pages keep working unchanged.
  How-Check: npx tsc --noEmit + all test suites (test:round/trend/criticality/dashboard/cache) exit 0; open all 5 pages at 1920px, 1366px and 375px widths → content spans the full width beside the sidebar, charts resize, no horizontal page scroll; collapse/expand the sidebar and reload → state remembered; on 375px the ☰ opens/closes the drawer; logo visible in the sidebar and the browser tab icon; no hydration errors in the console.
