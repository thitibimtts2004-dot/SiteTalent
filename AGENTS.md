...
```
[B1] Boot Init — DETERMINISTIC 2-form launcher (T-491 · C2). `boot_init.py` is the ONE cross-platform core (all boot logic); `boot_init.sh`/`boot_init.ps1` are thin shims that only resolve a Python interpreter + hand off (zero logic). Pick the form from the runtime you ALREADY know — the client Platform stated in this system prompt / detected.md `os:` (T-490) — never by guessing:
     • FORM A — bash (Mac/Linux/Git-Bash · the DEFAULT · use whenever a Bash tool exists): `BI=scripts/boot_init.sh; [ -f "$BI" ] || BI="$CLAUDE_PLUGIN_ROOT/scripts/boot_init.sh"; [ -f "$BI" ] || BI="$(ls -t ~/.claude/plugins/cache/*/harness-agent/*/scripts/boot_init.sh 2>/dev/null | head -1)"; { [ -f "$BI" ] && bash "$BI" || echo "[boot-FAILED] engine unreachable — boot_init.sh not found (local/plugin-root/plugin-cache all missed); harness enforcement may be OFF — run the manual checklist, do NOT assume ad-hoc is allowed"; }`  (resolve order: local `scripts/` → `$CLAUDE_PLUGIN_ROOT` → plugin-cache glob · `$CLAUDE_PLUGIN_ROOT` is EMPTY in a plain Bash tool call, so the glob fallback is what works for a plugin-only project (verified T-314) · the .sh shim execs boot_init.py, which self-locates ENGINE from its own file path so `$ENG` never 404s.)
     • FORM B — PowerShell (native Windows WITHOUT Git-Bash · `os:win32` + no Bash tool): self-hosted → `powershell -File scripts/boot_init.ps1` · plugin-only → `powershell -Command "& { $p = Get-ChildItem $HOME\.claude\plugins\cache\*\harness-agent\*\scripts\boot_init.ps1 -EA SilentlyContinue | Sort-Object LastWriteTime | Select-Object -Last 1; if ($p) { & $p.FullName } else { Write-Error '[boot-FAILED] engine unreachable — boot_init.ps1 not found; harness enforcement may be OFF — run the manual checklist, do NOT assume ad-hoc is allowed' } }"`. The .ps1 shim execs the SAME boot_init.py core (identical trace).
     Chicken-and-egg (which form, before boot has told you anything?): you already know the OS at turn 0 — it is the client Platform in this system prompt (and, once booted, detected.md `os:`). Choose FORM A when a Bash tool exists (Mac/Linux/Git-Bash); FORM B only for native Windows with no Bash. Running FORM A on a box with no `bash` fails LOUDLY (`bash: command not found` / the [boot-FAILED] line), a self-explaining first failure — install Git for Windows OR switch to FORM B — never a silent no-op. Both forms emit the same trace: [compact-restore] if any · active_thread tail · session_tokens · roadmap [/] · CFP_COUNT.
...
> **Constitution DETAIL is an engine read (T-348):** the same `<ENG>` rule applies to the constitution DETAIL docs — every `Implement/*.md` and `docs/session_templates/*.md` reference in this constitution is an ENGINE file, not project-local. Plugin-only project → Read them from `<ENG>` (e.g. `"<ENG>/Implement/03_config.md"`, `"<ENG>/docs/session_templates/mece_plan_schema.md"`); self-hosted → read as-is. Single source = the CLAUDE.md §R5 DOCS companion rule — no per-project copies (Model B · central engine).
```
...
[B4] Platform Probe: `detected.md` platform: unknown → list tools → update detected.md · else skip
     OS sub-probe (fills `os:` — run when `os:` missing OR =unknown · else skip · sibling of the provider sub-probe below): `python3 -c "import sys;p=sys.platform;print('win32' if p.startswith('win') else 'darwin' if p=='darwin' else 'linux')"` → write `os:` to detected.md. Deterministic · uses Python (NOT bash `uname`) so it resolves on native Windows too. WHY Python-not-bash + who reads `os:`: Implement/07_platform.md §How Detection Works (single source).
...
**Run C0→C1→C2→C3 before any work. No exceptions.** (C0 = 4-question gate; the token check is C0's Q3, formerly C0.5.)
> No task-size exception — see CLAUDE.md §Boot Gate (T-443). "Looks small / quick / conversational" is not a skip condition.
...
> **Dynamic fan-out (run-time · anywhere · T-481):** parallelism is NOT limited to a pre-declared Phase-3 cycle. At ANY point where ≥2 independent units are queued, the Orchestrator may run a 4-check gate — (1) ≥2 independent units · (2) no shared WRITE target (reads always parallel-safe) · (3) each unit ≥ a size floor (~≥3 tool calls) · (4) latency worth it — and if all pass, spawn a parallel batch with a barrier, REUSING the §14d cycle machinery (no new schema). A shared-WRITE output routes through **map-reduce** (parallel draft into own files → one serial merge). Single source: **Implement/06_orchestrator.md §14f**.
...
> **Frontier clarify (T-483):** G0 asks a NUMBERED *frontier* — only the questions answerable now, each with a recommended default — and stops when the frontier is EMPTY (completeness, not a round count), with `max_clarification_rounds: 5` kept as a SAFETY BACKSTOP. Single source: **Implement/03_config.md §G0** (no logic here — pointer only).
> **KB-discovery-first (T-435):** G1/G2 MUST include a KB consult — `python3 scripts/lookup.py <topic>` — and gather_complete.md MUST carry a `[kb-consulted]` line, or `phase_gate.py` blocks the first edit. Authoring surfaces (roadmap/plan/ticket) enforce the same read-before-write. SINGLE SOURCE → CLAUDE.md §Phase Transition (KB-discovery-first).
...
[M1] Read mece/SKILL.md → [M1.5] dependency_map + risk_flags + compact_checkpoint (≥3 sections → insert after ceil(N/2)) → [M2] build plan + Verify-N → [M3] Read docs/session_templates/mece_plan_schema.md → copy structure → fill task content → write gather_complete.md + write mece_plan.md (Phase 0-3 template mandatory · NEVER write from memory — CFP-019) → [M4] MANDATORY Skeptical Reviewer (auto · T-350 · runs the moment mece_plan.md is written · also greps knowledge/out_of_scope.md → already-rejected guard · appends on a permanent `reject` · T-224) → **Gate 3** auto-stamps `.sessions/.skeptical_ok` + HARD-BLOCKS the first Phase-3 edit if skeptical_reviewer was never loaded → emit `[sr-done] verdict:<go|revise|reject>` · FULL mechanism (auto-stamp · hash-tie · `[skeptical-gate] BLOCKED` · escape) → **CLAUDE.md §Review Gates** · canon **Implement/04_skills.md §MECE** → [M5] present plan to user → wait explicit confirm → [M6] roadmap: parent Task only (a NEW parent Task = full §6.2 block — Title/ContextTask/Goal/How-Check; schema: loop_engineer_spec.md §6.2 · NEVER per section — roadmap is big-task-only; usually already registered → just [X] at close) → [M7] emit [✓ MECE]
...
- [L4] mark mece_plan `[ ] S<N>` → `[X]` ONLY when `[✓ written]` AND Verify-N both pass (file write, not memory) AND a LIGHT per-section scrutinize ran (auto · T-350) → **Gate 4** auto-stamps `.sessions/.scrutinize_log` (`S<N>|<hash>`) + emits `[scrutinized S<N>]` + HARD-BLOCKS the `[X]` if scrutinize was never loaded · FULL scrutinize still runs once at close · FULL (today-set · `[scrutinize-gate] BLOCKED`) → **CLAUDE.md §Review Gates**
- [L4.8] independent correctness check (doer≠checker · auto · T-423): a **code_root** section spawns a FRESH-CONTEXT OUTSIDER to review the diff → `.sessions/codecheck_S<N>.json` (`verdict:confirm|concern`) BEFORE `[X]`; **Gate 5** (`skill_gate.py`) HARD-BLOCKS a missing/invalid confirm → `[codecheck-gate] BLOCKED` · non-code-root skips. FULL mechanism (Class A/B fail-closed · `[codecheck-machinery-down]` · `.codecheck_override` · escapes) = **CLAUDE.md §Review Gates** · canon **Implement/06_orchestrator.md §14d [L4.8]**. Pointer, not a re-definition.
- [L4.8b] behavioural Case-Test by DEFAULT (T-498/T-500): every non-trivial section declares `Behavior: assert|eval|none` (code_root DEFAULTS to assert · a refactor/mechanism code_root section may declare `none` + an audited `BehaviorReason:`); `Behavior: assert` HARD-BLOCKS `[X]` without a valid blind proof `.sessions/casetest_S<N>.json` (**Gate 6**) → `[casetest-gate] BLOCKED` · `eval` = non-blocking micro-eval. FULL rule = **CLAUDE.md §Review Gates** · SINGLE-SOURCE canon **knowledge/loop_engineer_spec.md §Behavioral Case Task**. Pointer, not a re-definition.
...
- **Pre-enforced (T-392 · `scripts/headroom_gate.py` · PreToolUse Bash|Read):** hard-blocks the 3 pre-sizable headroom cases (Door 1a `cat` big file · Door 1b known-verbose command · Door 2 `Read` >50KB no offset/limit) → route through `safe_run.py` / ranged-read · `.sessions/` exempt · `HARNESS_SKIP_HEADROOM_GATE=1` escape · headless→escalate · unpredictable output → T-344 nudge. Full rule (door mechanics · fail-open · size basis) = **CLAUDE.md §Never-Full-Load + Implement/03_config.md §R10** (single source).
...
- Close-gate (do NOT auto-close — CFP-037): first emit `[close-gate-check] trigger: (user typed /compact)=Y/N · (SESSION_TOTAL>80k)=Y/N · (LOOP_WEIGHT>50)=Y/N` (LOOP_WEIGHT from hook [token-state]; after T-235 session_tokens.md is subagent-clean too — either source is valid). All N → emit `[session-health]` + summary → WAIT for user · Any Y → proceed to close.
  > **T-452 auto-signal:** when the LAST Phase-3 section is marked `[X]`, `plan_lint.py` auto-emits a one-time `[task-closed]` ready-to-close signal (WARNs on missing handoff/compact_state · prompts Close-Checklist + /compact). Signal-only — it never writes phase:done / self-confirms (CFP-037). Full contract → `knowledge/session_close_loop.md` (single source).
...
> **Topic-assign DUTY (T-437):** creating or SUBSTANTIALLY editing a doc (`.md` only — never `.py`/`.json` · T-412) carries a duty, not just an index upsert. `mutation_sync.py` re-flags the doc `backfill_pending:True` (new doc, OR an edit ≥20% line-delta, OR still no topics); the AGENT then assigns topic+label **NOW** while the content is already in context (near-zero marginal cost) — **reuse-first** via `tag_gate.resolve_and_assign` (the SOLE resolver · register a NEW topic ONLY if genuinely new · NEVER hand-pick a topic id). Any doc still flagged at close is surfaced by `index_reconcile.py`: **interactive** → a `[tag-needed]` list + assign-now nudge; **headless** → escalated to `.sessions/review_queue/` (the next interactive session drains it) — **NEVER blind-mint headless, and NEVER trip loop_paused / block close** (a tagging gap is advisory, not destructive — no false deadlock · R14-headless).
Backlink check before editing: `backlinks[]` (every file that imports the one you edit — update them all) → **Implement/03_config.md §Backlink Rule**
...
→ **Run-time fan-out + map-reduce (T-481):** the cycle machinery is not Phase-3-only — the Orchestrator can invoke the 4-check fan-out gate ANYWHERE work is queued (≥2 independent units · no shared WRITE · size floor · latency worth it), and a shared-file write goes through map-reduce (parallel draft → serial merge). Single source: **Implement/06_orchestrator.md §14f** (extends this rule's trigger surface · depth=1 guardrail unchanged).
