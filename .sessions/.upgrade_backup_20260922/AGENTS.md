...
- [L4.8] independent correctness check (doer≠checker · auto · T-423): before marking `[X]` a section whose `File:` is under a **code_root** (`harness_paths.code_roots()`), the LOOP spawns a FRESH-CONTEXT OUTSIDER (delegate/spawn hat · no authorship bias · never "do-it-yourself-check-it-yourself") to review the diff for CORRECTNESS → the outsider writes `.sessions/codecheck_S<N>.json` (`verdict:confirm|concern`) → confirm→`[X]` · concern→fix + re-spawn a fresh checker · spawn-error (Class A)→re-spawn bounded → fail-closed (interactive ask / headless escalate) · never auto-pass (T-438 · was fail-open) · validator-down (Class B · skill_gate's own `import spawn_gate` fails → it can validate NO proof)→ALSO fail-closed: BLOCK + distinct `[codecheck-machinery-down]` marker + same escalate/escape ladder · remedy = FIX the environment, not re-spawn (T-439 · was fail-open-silent). On-record human escape (Class A fail-closed path only): **SCOPED per-section override (T-440)** — write `S<N>|<reason>` to `.sessions/.codecheck_override` → Gate 5 clears THAT section only, one-shot+TTL, stamps a greppable `verdict:override-unchecked` (never a real confirm), keeps Gate 3/4/T-263 live · coarse fallback `HARNESS_SKIP_REVIEW_GATE=1` (all gates). **Gate 5** (`skill_gate.py`, reuses `spawn_gate._valid_proof` so a hand-written proof can't satisfy it) HARD-BLOCKS the `[X]` without a valid confirm proof → `[codecheck-gate] BLOCKED`. Non-code-root sections skip this. Contract is SINGLE-SOURCE in **Implement/06_orchestrator.md §14d [L4.8]** — this is a pointer, not a re-definition.
